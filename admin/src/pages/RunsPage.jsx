...
import { CurrentStep, RunTimeline } from "../components/RunTimeline.jsx";
...
export function RunsPage({ status, settings, logs, onStart, onStop, onError }) {
  ...
  return <main className="page">
    ...
    <section className="run-config panel">
      ...
      {displayedConfig.mode === "enrich" ? <div className="security-notice"><Icon name="store" /><span><strong>Checkpoint enrichment.</strong> First lists every product the storefront still publishes, then reads only those pages — products switched off in the store are flagged and skipped instead of costing a failed request each. Adds dynamic diamond specifications and complete per-metal galleries, then updates the same master export. Keep Resume from checkpoint enabled.</span></div> : null}
...
function RunInspector({ run, current, logs, onStop }) {
  ...
  const storefront = (isCurrent ? current?.storefront : run.summary?.storefront) || null;
  return <aside className="run-inspector"><header><div><h2>Run inspector</h2><small>{run.run_id}</small></div><StatusMark state={run.state || "unknown"} /></header><div className="run-inspector__body"><dl className="metadata-grid"><div><dt>Started</dt><dd>{formatDate(run.started_at)}</dd></div><div><dt>Finished</dt><dd>{formatDate(run.finished_at)}</dd></div><div><dt>Mode</dt><dd>{config.mode || "—"}</dd></div><div><dt>Limit</dt><dd>{config.max_products || "Full"}</dd></div><div><dt>Categories</dt><dd>{config.category_ids?.length ? `${config.category_ids.length} selected` : "All"}</dd></div></dl>{isCurrent ? <section><h3>Steps</h3><CurrentStep status={current} /><RunTimeline timeline={current?.timeline} compact /></section> : null}{storefront ? <section><h3>Storefront availability</h3><div className="mini-coverage"><div><small>live on site</small><strong>{formatNumber(storefront.live_products)}</strong></div><div><small>skipped as off</small><strong>{formatNumber(storefront.skipped_offline)}</strong></div><div><small>newly off</small><strong>{formatNumber(storefront.newly_offline)}</strong></div><div><small>back online</small><strong>{formatNumber(storefront.back_online)}</strong></div></div>{storefront.scanned === false ? <p className="catalog-discovery__note">The storefront index could not be read, so every saved product was tried.</p> : null}</section> : null}<section><h3>Data coverage</h3><div className="mini-coverage">{["products", "variations", "images", "diagnostics"].map((key) => <div key={key}><small>{key}</small><strong>{formatNumber(counts[key])}</strong></div>)}</div></section>{isCurrent && (run.state === "running" || run.state === "stopping") ? <Button icon="stop" tone="danger" onClick={onStop}>Stop run</Button> : null}<section><h3>Live log</h3><pre className="live-log">{isCurrent ? logs || "Waiting for crawler output…" : "Historical logs are included in the run export."}</pre></section></div></aside>;
}
